...
def evaluate_all(problems, client, model, max_retries=MAX_RETRIES, progress_cb=None):
    ...
    summary = {
        ...
        "agent": {
            "pass": agent_pass,
            "total": total,
            "pass_at_1": round(agent_pass / total * 100, 1),
            "by_difficulty": by_diff(agent_res),
            "avg_attempts": round(sum(r["attempts"] for r in agent_res) / total, 2),
        },
        "baseline": {
            "pass": base_pass,
            "total": total,
            "pass_at_1": round(base_pass / total * 100, 1),
            "by_difficulty": by_diff(base_res),
        },
    ...
    # TODO Day 8: write to results/humaneval_DATE.json (committed to git)
    # instead of logs/ (gitignored)
